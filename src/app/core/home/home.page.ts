...
export class HomePage implements OnInit, OnDestroy {
  ...
  public recordCityLabel: { [cityId: string]: City } = {}
  ...
  get focusedCityName(): Observable<string> {
    return this.userSettings$.pipe(
      select(
        (userSettings: AppUserParams) => {
          return userSettings.listCity.find(
            (city) => city.id === this.listCityLabel?.[this.idFocusedCity]
          )?.name
        }
      )
    );
